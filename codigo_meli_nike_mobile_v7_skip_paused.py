...
def scrape_meli_pdp_with_retry(page, mla: str, nike_full: float, attempt: int = 1) -> Dict[str, Any]:
    ...
    try:
        ...
        if not out["is_buyable"]:
            out["status"] = "not_buyable"
            out["error_detail"] = "PDP no comprable (buybox ausente/disabled o texto de no disponible)"
            log_warning("  ⚠️ PDP NO comprable (posible pausada/sin stock). Se omite para evitar cross-selling.")
            return out
