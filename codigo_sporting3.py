...
SPORTING_PLP_URLS = [
    ...
    "https://www.sporting.com.ar/nike?initialMap=b&initialQuery=nike&map=category-3,brand&query=/zapatillas/nike&searchState",
    "https://www.sporting.com.ar/nike?initialMap=b&initialQuery=nike&map=category-3,category-3,category-3,category-3,brand&query=/botines/guantes/pantalones/remeras/nike&searchState",
]

CP_AMBA = "1425"

SPORTING_STD_SHIPPING_ARS = 10900.0
SPORTING_FREE_SHIP_FROM_ARS = 149999.0
...
OUT_RAW_CSV = f"sporting_vs_nike_raw_{TS}.csv"
OUT_VISUAL_XLSX = f"sporting_vs_nike_visual_{TS}.xlsx"
...
def extract_stylecolor_from_url(url: str) -> str:
    """
    Sporting URLs reales:
      - /zapatillas-nike-downshifter-13-de-hombre-fd6454001-001/  -> FD6454001-001
      - /zapatillas-nike-court-vision-low-de-mujer-cd5434101/     -> CD5434101
    El stylecolor esta embebido al FINAL del slug.
    Patron 1: letras(0-2) + digitos(6-9) + guion + alfanum(2-6)  => con sufijo de color
    Patron 2: letras(0-2) + digitos(6-9)                          => sin sufijo (un solo bloque)
    """
    path = url.split("?")[0].rstrip("/")
    # Patron 1: stylecolor con sufijo, ej: FD6454001-001, DH2987101-100
    m = re.search(r"-([A-Za-z]{0,2}[0-9]{6,9}-[A-Za-z0-9]{2,6})(?:/|$)", path)
    if m:
        return m.group(1).upper()
    # Patron 2: stylecolor sin sufijo, ej: CD5434101, DV4342061
...
# ─────────────────────────────────────────────────────────────────
# PLP — recoleccion de links
# ─────────────────────────────────────────────────────────────────
def collect_plp_links_sporting(page) -> list:
    """
    ...
    if not links:
        ...
        for h in items[:10]:
            ...
            if hs:
                sample.append(hs)
        log(f"   ⚠️ collect_plp_links_sporting: anchors vistos={total_anchors}, links válidos=0")
        if sample:
            log(f"   ⚠️ sample hrefs: {sample[:5]}")

    return links


def _click_mostrar_mas(page) -> bool:
    """Clickea el boton 'MOSTRAR MAS' de Sporting."""
    try:
        btn = page.locator("button", has_text=re.compile(r"mostrar\s+m[aá]s", re.I)).first
        if btn.count() == 0:
            btn = page.locator("text=/mostrar\\s+m[aá]s/i").first
        if btn.count() == 0:
            return False
...
def collect_pdp_links_from_plp(page, plp_url: str) -> list:
    """
    Estrategia combinada: scroll + click en 'Mostrar mas'.
    Primero barre el DOM con scroll para lazy-load,
    luego clickea 'Mostrar mas' y repite hasta estancamiento.
    """
    ...
    stagnation = 0
    prev_total = 0
    mostrar_mas_clicks = 0

    for round_i in range(1, MAX_PLP_SCROLL_ROUNDS + 1):
        ...
        # Recolectar links en el DOM actual
        links_now = collect_plp_links_sporting(page)
        new_added = 0
        for u in links_now:
            if u not in seen:
                seen.add(u)
        ...
        total = len(seen)
        log(f"   [Round {round_i}] PDPs acumulados: {total} (+{new_added})")

        if total <= prev_total and new_added == 0:
            stagnation += 1
        else:
            stagnation = 0
        prev_total = total
        ...
        if mostrar_mas_clicks < MAX_MOSTRAR_MAS_CLICKS:
            clicked = _click_mostrar_mas(page)
            if clicked:
                ...
                while time.time() - t0 < 20:
                    ...
                    if len(collect_plp_links_sporting(page)) >= target:
                        grew = True
                        break
                # Solo resetear stagnation si realmente crecieron los productos
                if grew:
                    stagnation = 0
                else:
                    log(f"   ⚠️ Click #{mostrar_mas_clicks} no produjo nuevos productos.")
                    stagnation += 1
                continue  # volver a scroll para recolectar

        if stagnation >= PLP_STAGNATION_ROUNDS:
            log(f"   ✅ Stop PLP: estancada {stagnation} rondas sin nuevos productos.")
            break

    log(f"\n✅ PLP completa: {len(all_links)} PDPs unicos recolectados.")
    return all_links


# ─────────────────────────────────────────────────────────────────
# PDP — extraccion de precios y cuotas
# ─────────────────────────────────────────────────────────────────
def _extract_sporting_prices(page) -> tuple:
    """
    Retorna (final_price, full_price).
    Capas: 1) DOM VTEX clases especificas  2) JSON-LD  3) Meta tags  4) DOM generico
    """
    # CAPA 1: DOM estructurado VTEX de Sporting
    try:
        prices = page.evaluate(
            """
            () => {
                function parseArPrice(txt) {
                    // Formatos AR: "$\u00a0126.000" / "$126.000,50" / "$ 126000"
                    if (!txt) return 0;
                    // Quitar simbolo $ y espacios
                    let s = txt.replace(/[$\\u00a0\\s]/g, "");
                    // Si tiene coma: es separador decimal AR -> quitar puntos de miles, coma=decimal
                    if (s.includes(",")) {
                        s = s.replace(/\\./g, "").replace(",", ".");
                    } else {
                        // Sin coma: los puntos son separadores de miles -> quitarlos
                        s = s.replace(/\\./g, "");
                    }
                ...
                function extractFromContainer(el) {
                    if (!el) return 0;
                    // Intentar primero el innerText completo del contenedor
                    const full = (el.innerText || el.textContent || "").trim();
                    const v = parseArPrice(full);
                    if (v > 500) return v;  // precio minimo razonable en ARS
                    // Fallback: buscar span con $ dentro
                    for (const span of el.querySelectorAll("span,div")) {
                        const t = (span.innerText || "").trim();
                        if (t.includes("$")) {
                            const vv = parseArPrice(t);
                            if (vv > 500) return vv;
                        }
                    }
                    return 0;
                ...
                for (const sel of sellingSelectors) {
                    ...
                    if (el) {
                        const v = extractFromContainer(el);
                        if (v > 0) { finalPrice = v; break; }
                    }
                }

                const listSelectors = [
                    '[class*="customPdpListPrice"]',
                    '[class*="ListPrice"]',
                    '[class*="listPrice"]',
                    '[class*="hasListPrice"]',
                ];
                for (const sel of listSelectors) {
                    ...
                    if (el) {
                        const v = extractFromContainer(el);
                        if (v > 0 && v >= finalPrice) { fullPrice = v; break; }
                    }
                }

                // Buscar precio tachado si aun no tenemos full price
                if (fullPrice === 0 && finalPrice > 0) {
                    const dels = Array.from(document.querySelectorAll(
                        "del, s, [class*='strike'], [class*='Strike']"
                    ));
                    for (const el of dels) {
                        const v = parseArPrice(el.innerText || "");
                        if (v > finalPrice) { fullPrice = v; break; }
                    }
                }
    ...
    # CAPA 4: DOM generico (mayor precio visible sin tachado)
    try:
        vals = page.evaluate(
            """
            () => {
        ...
        if vals:
            ...
            filtered = []
            for s in vals:
                if any(t in str(s).lower() for t in bad_tokens):
                    continue
...
def extract_max_cuotas_sporting(page) -> int:
    """
    Extrae maximo de cuotas sin interes. Acepta 'en bancos seleccionados' como valido.
    """
    try:
        page.wait_for_selector(
            "[class*='installment'], [class*='Installment'], [class*='cuota'], [class*='payment']",
            timeout=3000,
        )
    except Exception:
        pass
...
def scrape_pdp(page, url: str) -> dict:
    ...
    full_price = 0.0
    for _try in range(2):
        price_final, full_price = _extract_sporting_prices(page)
        if price_final > 0:
            break
        try:
    ...
    max_cuotas = extract_max_cuotas_sporting(page)
    ...
    return {
        ...
        "Sporting_URL": url,
        "Sporting_Name": name,
        "Sporting_StyleColor_Raw": style_raw,
        "Sporting_StyleColor_Norm": style_norm,
        "Sporting_FullPrice": float(full_price),
        "SportingSaleDecimal": float(sale_dec),
        "Sporting_FinalPrice": float(price_final),
        "Sporting_MaxCuotasSinInteres": int(max_cuotas),
        "Sporting_LastUpdated": dt.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
    }
...
def _ship_sporting(price: float) -> float:
    try:
        p = float(price)
    except Exception:
        return 0.0
    if p <= 0:
        return 0.0
    return 0.0 if p >= SPORTING_FREE_SHIP_FROM_ARS else SPORTING_STD_SHIPPING_ARS
...
def build_template_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    def _sanitize_nike_price(col):
        ...
        def _fix(row):
            try:
                comp = float(row.get("Sporting_FinalPrice") or 0)
            except Exception:
                comp = 0.0
            try:
                raw = row.get(col)
    ...
    base_cols = [
        "Sporting_StyleColor_Norm", "SB_ProductCode", "SB_MarketingName", "SB_Category",
        "SB_Division", "SB_Franchise", "PLP_PrimaryCategoria", "Sporting_URL",
        "Sporting_FullPrice", "SportingSaleDecimal", "Sporting_FinalPrice",
        "NikeFullPrice", "NikeSaleDecimal", "NikeFinalPrice",
        "Sporting_MaxCuotasSinInteres", "Sporting_LastUpdated",
    ...
    df["StyleColor"] = df["Sporting_StyleColor_Norm"].astype(str).fillna("").str.strip()
    ...
    df["Link PDP Competitor"] = df["Sporting_URL"].astype(str).fillna("").str.strip()

    df["Competitor Full Price"] = pd.to_numeric(df["Sporting_FullPrice"], errors="coerce")
    df["Competitor Markdown"] = pd.to_numeric(df["SportingSaleDecimal"], errors="coerce")
    df["Competitor Final Price"] = pd.to_numeric(df["Sporting_FinalPrice"], errors="coerce")
    ...
    df["Competitor Shipping"] = df["Competitor Final Price"].apply(
        lambda x: _ship_sporting(float(x)) if pd.notna(x) else 0.0
    )
    ...
    df["Cuotas Competitor"] = pd.to_numeric(
        df["Sporting_MaxCuotasSinInteres"], errors="coerce"
    ).fillna(0).astype(int)
    ...
    df["Competitor"] = "Sporting"
    df["Last Update Competitor"] = df["Sporting_LastUpdated"].astype(str).fillna("").str.strip()
...
# ─────────────────────────────────────────────────────────────────
def main():
    log("🚀 SPORTING vs NIKE – INDESTRUCTIBLE MODE")
    ...
    log(f"\n📦 Stage 1: recolectando PDPs desde {len(SPORTING_PLP_URLS)} PLPs ...")
    style_meta = {}

    with sync_playwright() as pw:
        browser, context, page = new_browser_context(pw)
        for plp_url in SPORTING_PLP_URLS:
            try:
                links = collect_pdp_links_from_plp(page, plp_url)
            except PWTimeoutError:
                log(f"⚠️ Timeout en PLP {plp_url}. Continuo con la siguiente.")
                links = []
            except Exception as e:
                log(f"⚠️ Error en PLP {plp_url}: {e}. Continuo con la siguiente.")
                links = []
            ...
            log(f"   ✅ Acumulado tras PLP '{plp_url}': {len(style_meta)} stylecolors únicos")
        ...
        with sync_playwright() as pw:
            ...
            for style_norm in styles_chunk:
                ...
                with cache_lock:
                    ...
                    cache[style_norm]["Sporting_URL"] = url
                    cache[style_norm]["Sporting_StyleColor_Norm"] = style_norm
                ...
                if (not REFRESH_CACHED) and already_has_valid:
                    ...
                    try:
                        ...
                        sport_price = float((row or {}).get("Sporting_FinalPrice", 0.0))

                        with cache_lock:
                            for k, v in row.items():
                                cache[style_norm][k] = v
                            cache[style_norm]["Sporting_PriceValid"] = bool(sport_price > VALID_PRICE_MIN)
                            _nk = cache[style_norm].get("NikeFinalPrice", 0.0)
                        ...
                        log(
                            f"   🧾 SportingFinal={sport_price:,.2f} | NikeFinal={nike_display} | "
                            f"{delta_str} | Cuotas={cache.get(style_norm, {}).get('Sporting_MaxCuotasSinInteres', 0)}"
                        )
    ...
    if not df.empty and "Sporting_StyleColor_Norm" in df.columns:
        df = df.sort_values("Sporting_StyleColor_Norm").reset_index(drop=True)
