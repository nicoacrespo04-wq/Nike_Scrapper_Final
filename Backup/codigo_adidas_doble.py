...
#
# Proxies:
#   - AR : gate.decodo.com:10001-10010  (Playwright residencial)
...
# Nombres de producto bloqueados / invalidos
BLOCKED_NAMES = {"account-portal-disable", "account portal disable"}

# Timeouts
TO_NAV_AR         = 35_000
TO_WARMUP_AR      = 40_000
TO_NEXT_DATA_AR   = 18    # segundos
TO_REQ_US         = 45    # segundos
...
def safe_float(x) -> Optional[float]:
    try:
        if x is None:
            return None
        if isinstance(x, str):
            # Intentar parsear directo (punto como decimal)
            s2 = x.strip().replace("$", "").replace(",", "").strip()
            try:
                v = float(s2)
                return None if math.isnan(v) else v
            except Exception:
                pass
            # Formato ARS con punto como separador de miles
...
def bml(ar_val: Optional[float], us_adj: Optional[float]) -> str:
    if ar_val is None or us_adj is None or us_adj == 0:
        return "N/D"
    diff = ar_val / us_adj - 1
    if abs(diff) <= BML_TOL:
        return "MEET"
    return "BEAT" if diff < -BML_TOL else "LOSE"


def shipping_adidas_ar(price_ars: Optional[float]) -> float:
    if price_ars is None:
        return ADIDAS_SHIP_ARS
    return 0.0 if price_ars > ADIDAS_FREE_ARS else ADIDAS_SHIP_ARS


def shipping_nike_ar(price_ars: Optional[float]) -> float:
    if price_ars is None:
        return NIKE_SHIP_ARS
    return 0.0 if price_ars > NIKE_FREE_ARS else NIKE_SHIP_ARS


def shipping_us(price_usd: Optional[float]) -> float:
    if price_usd is None:
        return ADIDAS_SHIP_USD
    return 0.0 if price_usd >= ADIDAS_FREE_USD else ADIDAS_SHIP_USD
...
def get_fx() -> float:
    ...
    url = endpoint_map.get(FX_MODE, endpoint_map["oficial"])
    try:
        ...
        log(f"   FX API error: {e} — usando fallback {FX_FALLBACK}")
    return FX_FALLBACK
...
def load_statusbooks(path: str, season: str) -> pd.DataFrame:
    log(f"Cargando StatusBooks [{path}]...")
    ...
    log(f"   OK {len(df):,} filas con precio > 0")
    return df


def load_comparativa(path: str) -> pd.DataFrame:
    log(f"Cargando Comparativa [{path}]...")
    ...
    for c in df.columns:
        cu = c.upper().strip()
        if cu in ("CATEGORIA", "CATEGORÍA", "CATEGORY"):
            col_map[c] = "Categoria"
        elif cu == "NIKE":
            col_map[c] = "NIKE"
        elif cu == "ADIDAS":
    ...
    required = ["Categoria", "NIKE", "ADIDAS"]
    missing = [c for c in required if c not in df.columns]
    if missing:
        raise RuntimeError(
            f"Comparativa missing cols: {missing}. Encontradas: {list(df.columns)}"
        )
    ...
    log(f"   OK {len(df)} pares Nike-Adidas")
    return df


# ─────────────────────────────────────────────────────────────────
# LÓGICA DE FRANQUICIAS Y MATCHING
# ─────────────────────────────────────────────────────────────────
def parse_franchise_tokens(franchise: str) -> List[str]:
    """
    Extrae tokens requeridos de una franquicia.
    Lógica de comillas: palabras entre "" son tokens adicionales no necesitan
    estar juntos a los otros pero SI deben aparecer en el texto.
    Ejemplo: 'ADIZERO "BOSTON"' -> tokens = ['adizero', 'boston']
             'SAMBA' -> tokens = ['samba']
             'FORUM LOW' -> tokens = ['forum', 'low']
    """
...
def franchise_matches_text(text: str, tokens: List[str]) -> bool:
    """Todos los tokens deben aparecer en el texto (cualquier orden)."""
    t = text.lower()
    return all(tok in t for tok in tokens)


def find_nike_stylecolor(
    nike_franchise: str, df_sb: pd.DataFrame
) -> Tuple[Optional[str], Optional[str]]:
    """
    Busca en el StatusBooks la primera fila cuya columna Franchise
    matchea con la franquicia Nike dada.
    Retorna (stylecolor / product_code, marketing_name).
    """
...
def score_product(product_name: str, franchise: str) -> float:
    """
    Score de relevancia de un producto respecto a la franquicia.
    Palabras entre comillas en la franquicia cuentan igual que las demás.
    """
    if is_blocked_name(product_name):
        return -999.0
    name_lower = product_name.lower()
    tokens = parse_franchise_tokens(franchise)
    if not tokens:
        return 0.0
    score = sum(2.0 for tok in tokens if tok in name_lower)
    score += 0.5  # bonus precio presente
    return score


def pick_best_product(products: List[dict], franchise: str) -> Optional[dict]:
    """Elige el mejor producto, descartando los bloqueados."""
    if not products:
        return None
    valid = [p for p in products if not is_blocked_name(p.get("name", ""))]
    if not valid:
        log(f"   Todos bloqueados (ACCOUNT-PORTAL-DISABLE)")
        return None
    scored = sorted(valid, key=lambda p: score_product(p.get("name", ""), franchise), reverse=True)
    best = scored[0]
    s = score_product(best.get("name", ""), franchise)
    cur = best.get("currency", "?")
    pr  = best.get("full_price", 0)
    log(f"   Best: '{best.get('name','')[:50]}' | score={s:.1f} | {cur} {pr:,.0f}")
    return best
...
FRANCHISE_URLS_AR: Dict[str, List[str]] = {
    ...
    "SPEEDPORTAL":         ["botines-x_speedportal", "x_speedportal", "botines"],
    "CLUB":                ["botines-copa_pure", "copa_pure", "botines"],
    "LEAGUE":              ["botines-copa_pure", "copa_pure", "botines"],
...
FRANCHISE_URLS_US: Dict[str, List[str]] = {
    ...
    "SPEEDPORTAL":         [f"{BASE_US}/x_speedportal-soccer-shoes", f"{BASE_US}/copa_pure-soccer-shoes"],
    "CLUB":                [f"{BASE_US}/copa_pure_ii-soccer-shoes", f"{BASE_US}/copa_pure-soccer-shoes"],
    "LEAGUE":              [f"{BASE_US}/copa_pure-soccer-shoes"],
...
def get_urls_ar(franchise: str) -> List[str]:
    ...
    for key, urls in FRANCHISE_URLS_AR.items():
        if re.sub(r'["\']', '', key).strip().upper() == clean.upper():
            return urls
    slug = re.sub(r'["\' ]+', '_', clean.lower()).strip('_')
...
# ─────────────────────────────────────────────────────────────────
async def build_ar_browser(pw):
    port  = random.choice(AR_PORTS)
...
async def warmup_ar(page) -> bool:
    for attempt in range(2):
        try:
            ...
            log(f"   Warmup AR fallo (intento {attempt+1}): {str(e)[:60]}")
...
async def wait_next_data_ar(page, max_wait: int = TO_NEXT_DATA_AR) -> List[dict]:
    ...
    while time.time() < deadline:
        ...
        try:
            ...
            if products:
                log(f"   __NEXT_DATA__ AR OK — {len(products)} productos (intento {attempt})")
                return products
            log(f"   vacio (intento {attempt})")
        except Exception as e:
            log(f"   eval error: {str(e)[:60]}")
        await asyncio.sleep(1.5)
    log(f"   __NEXT_DATA__ timeout {max_wait}s")
    return []


async def scrape_ar_franchise(page, franchise: str) -> Optional[dict]:
    ...
    for slug in slugs:
        ...
        log(f"\n   AR -> {url}")
        try:
            # "commit" = dispara apenas llegan los primeros bytes del servidor
            # NO espera DOM completo — el __NEXT_DATA__ polling se encarga del resto
            # Esto era la clave en el debug original (22/24 en 4 min)
            ...
            # Si aterrizó en categoría amplia, filtrar por tokens
            if slug in ("zapatillas", "zapatillas-running", "botines"):
                tokens   = parse_franchise_tokens(franchise)
            ...
            log(f"   Timeout AR: {url}")
        except Exception as e:
            err = str(e)[:100]
            log(f"   Error AR: {'ERR_TUNNEL' if 'ERR_TUNNEL' in err else err}")
...
# ─────────────────────────────────────────────────────────────────
US_PROXY_URL = f"http://{UB_USER}:{UB_PASS}@{UB_HOST}:{UB_PORT}"
US_PROXIES   = {"http": US_PROXY_URL, "https": US_PROXY_URL}
US_HEADERS   = {
...
def scrape_us_franchise(franchise: str) -> Optional[dict]:
    ...
    for url in urls:
        ...
        try:
            ...
            if products:
                log(f"   US {len(products)} productos")
                best = pick_best_product(products, franchise)
                if best:
                    return best
            else:
                log("   sin __NEXT_DATA__ US")
        except requests.exceptions.Timeout:
            log(f"   US Timeout")
        except Exception as e:
            log(f"   US Error: {str(e)[:100]}")
...
async def scrape_all(franchises: List[str]) -> Dict[str, Dict]:
    ...
    async def run_ar(pw):
        ...
        try:
            browser, context, page = await build_ar_browser(pw)
            ok = await warmup_ar(page)
            if not ok:
                log("Warmup AR fallo — reintentando con nuevo browser...")
                try:
                    await browser.close()
                except Exception:
                    pass
            ...
            for i, fr in enumerate(franchises):
                ...
                results[fr]["ar"] = await scrape_ar_franchise(page, fr)
...
def build_rows(
    ...
    for _, comp_row in comparativa.iterrows():
        ...
        ar_product = scrape_results.get(adidas_fr, {}).get("ar")
        us_product = scrape_results.get(adidas_fr, {}).get("us")

        adidas_ar_price = ar_product.get("full_price") if ar_product else None
        adidas_ar_name  = ar_product.get("name", "")   if ar_product else ""
        adidas_ar_url   = ar_product.get("url", "")    if ar_product else ""
        adidas_us_price = us_product.get("full_price") if us_product else None
        adidas_us_name  = us_product.get("name", "")   if us_product else ""

        # Nike data del StatusBooks
        stylecolor, mk_name = find_nike_stylecolor(nike_fr, df_sb)
        nike_price_ars: Optional[float] = None
        if stylecolor:
            match = df_sb[df_sb["Product Code"].str.upper() == stylecolor.upper()]
            if not match.empty:
                nike_price_ars = safe_float(match.iloc[0].get("_price"))
        mk_name = mk_name or ""

        # ── OUTPUT 1: Nike AR vs Adidas AR ─────────────────────────
        nike_shp       = shipping_nike_ar(nike_price_ars)
        adidas_ar_shp  = shipping_adidas_ar(adidas_ar_price)
        nike_total     = (nike_price_ars  + nike_shp)    if nike_price_ars  is not None else None
        adidas_total   = (adidas_ar_price + adidas_ar_shp) if adidas_ar_price is not None else None

        gap_price  = (
            (adidas_ar_price - nike_price_ars) / nike_price_ars
            if adidas_ar_price is not None and nike_price_ars and nike_price_ars > 0 else None
        )
        gap_shp = (
            (adidas_total - nike_total) / nike_total
            if adidas_total is not None and nike_total and nike_total > 0 else None
        )
        ...
        rows1.append({
            ...
            "Nike Full Price (ARS)":   nike_price_ars,
            "Adidas Full Price (ARS)": adidas_ar_price,
            "Gap %":                   gap_price,
            "BML Full Price":          bml(adidas_ar_price, nike_price_ars),
            "Nike Shipping (ARS)":     nike_shp       if nike_price_ars  is not None else None,
            "Adidas Shipping (ARS)":   adidas_ar_shp  if adidas_ar_price is not None else None,
            "Gap % con Shipping":      gap_shp,
            "BML + Shp":               bml(adidas_total, nike_total),
            "Adidas Product Name":     adidas_ar_name,
            "Adidas PDP (AR)":         adidas_ar_url,
        })

        # ── OUTPUT 2: Adidas AR vs Adidas US ───────────────────────
        ar_usd       = adidas_ar_price / fx                    if adidas_ar_price is not None and fx > 0 else None
        us_iva       = adidas_us_price * (1 + IVA)             if adidas_us_price is not None else None
        us_iva_bf    = adidas_us_price * (1 + IVA) * (1 + BF) if adidas_us_price is not None else None
        shp_ar_usd   = shipping_adidas_ar(adidas_ar_price) / fx if fx > 0 else 0.0
        shp_us_usd   = shipping_us(adidas_us_price)
        ar_plus_shp  = (ar_usd + shp_ar_usd)                   if ar_usd is not None else None
        us_plus_shp  = (adidas_us_price + shp_us_usd) * (1 + IVA) * (1 + BF) if adidas_us_price is not None else None

        dif_fp       = ar_usd / adidas_us_price - 1 if ar_usd and adidas_us_price else None
        dif_iva      = ar_usd / us_iva - 1          if ar_usd and us_iva           else None
        dif_iva_bf   = ar_usd / us_iva_bf - 1       if ar_usd and us_iva_bf        else None
        dif_shp      = ar_plus_shp / us_plus_shp - 1 if ar_plus_shp and us_plus_shp else None

        rows2.append({
            "Fecha":                    run_date,
            "Categoria":                categoria,
            "Franchise":                adidas_fr,
            "Retail ARG (ARS)":         adidas_ar_price,
            "FX ARS/USD":               fx,
            "ARG (USD)":                ar_usd,
            "USA Full (USD)":           adidas_us_price,
            "Dif FP vs USA":            dif_fp,
            "USA + 21% IVA":            us_iva,
            "Dif FP + IVA":             dif_iva,
            "BML c/ IVA":               bml(ar_usd, us_iva),
            "USA + 21% + BF 8% (USD)":  us_iva_bf,
            "Dif FP + IVA + BF":        dif_iva_bf,
            "BML c/ IVA + BF":          bml(ar_usd, us_iva_bf),
            "AR + Shp":                 ar_plus_shp,
            "US + Shp":                 us_plus_shp,
            "Dif + Shp":                dif_shp,
            "BML + Shp":                bml(ar_plus_shp, us_plus_shp),
            "Adidas AR Product Name":   adidas_ar_name,
            "Adidas US Product Name":   adidas_us_name,
        })
...
# ─────────────────────────────────────────────────────────────────
COLS_OUT1 = [
    "Fecha", "Categoria", "StyleColor", "Marketing Name (Nike)", "Franchise Adidas",
...
def _write_sheet(wb, fmts, sheet_name: str, rows: List[Dict], cols: List[str], bml_cols: set):
    ...
    fx_cols   = {"FX ARS/USD"}
    link_cols = {c for c in cols if "PDP" in c or "Link" in c or "URL" in c}
    ...
    for i, row in enumerate(rows, start=1):
        ...
        for j, c in enumerate(cols):
            ...
            if c in link_cols:
                ...
                ws.write_number(i, j, float(v), fmts["ars"]) if v is not None else ws.write(i, j, "", fmts["txt"])
            elif c in usd_cols:
                ws.write_number(i, j, float(v), fmts["usd"]) if v is not None else ws.write(i, j, "", fmts["txt"])
            elif c in pct_cols:
                ws.write_number(i, j, float(v), fmts["pct"]) if v is not None else ws.write(i, j, "", fmts["txt"])
            elif c in fx_cols:
                ws.write_number(i, j, float(v), fmts["fx"])  if v is not None else ws.write(i, j, "", fmts["txt"])
            else:
...
async def main():
    ...
    # 2. Franquicias únicas de Adidas
    franchises = list(comparativa["ADIDAS"].dropna().unique())
    log(f"\n{len(franchises)} franquicias Adidas: {franchises}")

    # 3. Scraping paralelo
    log("\n" + "="*60)
    log("Iniciando scraping AR + US en paralelo...")
    log("="*60)
    ...
    ok1  = sum(1 for r in rows1 if r.get("Adidas Full Price (ARS)") is not None)
    ok2  = sum(1 for r in rows2 if r.get("USA Full (USD)") is not None)
    fail = sum(1 for r in rows1 if r.get("Adidas Full Price (ARS)") is None)

    log("\n" + "="*60)
    log(f"Completado en {elapsed/60:.1f} min")
    log(f"Output 1 — Nike AR vs Adidas AR : {ok1}/{len(rows1)} con precio Adidas AR ({fail} sin dato)")
    log(f"Output 2 — Adidas AR vs US      : {ok2}/{len(rows2)} con precio Adidas US")
    log(f"Excel  : {out_xlsx}")
    log(f"CSV 1  : {out_csv1}")
    log(f"CSV 2  : {out_csv2}")
    log("="*60)
    ...
    for r1, r2 in zip(rows1, rows2):
        ...
        log(f"  {fr:25s} | {ar_s} | {us_s} | BML-AR:{b1:4s} | BML-US:{b2:4s}")
