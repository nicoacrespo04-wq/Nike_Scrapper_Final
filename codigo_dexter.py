...
DEXTER_STD_SHIPPING_ARS = 6899.0
DEXTER_FREE_SHIP_FROM_ARS = 169000.0
...
OUT_RAW_CSV = f"dexter_vs_nike_raw_{TS}.csv"
OUT_VISUAL_XLSX = f"dexter_vs_nike_visual_{TS}.xlsx"
...
def scrape_pdp(page, url: str) -> dict:
    ...
    return {
        ...
        "Dexter_URL": url,
        "Dexter_Name": name,
        "Dexter_StyleColor_Raw": style_raw,
        "Dexter_StyleColor_Norm": style_norm,
        "Dexter_FullPrice": float(full_price),
        "DexterSaleDecimal": float(sale_dec),
        "Dexter_FinalPrice": float(price_final),
        "Dexter_MaxCuotasSinInteres": int(max_cuotas),
        "Dexter_LastUpdated": dt.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
    }
...
def _ship_for_price_dexter(price: float) -> float:
    try:
        p = float(price)
    except Exception:
        return 0.0
    if p <= 0:
        return 0.0
    try:
        return 0.0 if p >= float(DEXTER_FREE_SHIP_FROM_ARS) else float(DEXTER_STD_SHIPPING_ARS)
    except Exception:
        return 0.0
...
def build_template_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    base_cols = [
        'Dexter_StyleColor_Norm','SB_ProductCode', 'SB_MarketingName', 'SB_Category','SB_MarketingName','SB_Division','SB_Franchise',
        'PLP_PrimaryCategoria','Dexter_URL',
        'Dexter_FullPrice','DexterSaleDecimal','Dexter_FinalPrice',
        'NikeFullPrice','NikeSaleDecimal','NikeFinalPrice', 'FechaCorrida',
        'Dexter_MaxCuotasSinInteres','Dexter_LastUpdated','SB_SSN_VTA','Aux_Año','InStatusBooks'
    ]
    for c in base_cols:
        if c not in df.columns:
            df[c] = pd.NA

    df['StyleColor'] = df['Dexter_StyleColor_Norm'].astype(str).fillna('').str.strip()
    ...
    df['Link PDP Competitor'] = df['Dexter_URL'].astype(str).fillna('').str.strip()

    df['Dexter Full Price'] = pd.to_numeric(df['Dexter_FullPrice'], errors='coerce')
    df['Dexter Markdown'] = pd.to_numeric(df['DexterSaleDecimal'], errors='coerce')
    df['Dexter Final Price'] = pd.to_numeric(df['Dexter_FinalPrice'], errors='coerce')

    _m_full_missing = df['Dexter Full Price'].isna() | (df['Dexter Full Price'] <= 0)
    if _m_full_missing.any():
        df.loc[_m_full_missing, 'Dexter Full Price'] = df.loc[_m_full_missing, 'Dexter Final Price']
        df.loc[_m_full_missing, 'Dexter Markdown'] = 0.0
    ...
    dex_full = df['Dexter Full Price'].replace(0, pd.NA)
    df['Dexter vs Nike (Full Price)'] = (df['Dexter Full Price'] - df['Nike Full Price']) / dex_full

    nike_full_den = df['Nike Full Price'].replace(0, pd.NA)
    diff_pct_full_nike = (df['Dexter Full Price'] - df['Nike Full Price']) / nike_full_den
    df['Dexter vs Nike Full (BML)'] = df.apply(lambda r: _bml_label_from_prices(r.get('Dexter Full Price'), r.get('Nike Full Price')), axis=1)

    dex = df['Dexter Final Price'].replace(0, pd.NA).replace(0, pd.NA)
    df['Dexter vs Nike'] = (df['Dexter Final Price'] - df['Nike Final Price']) / dex

    nike = df['Nike Final Price'].replace(0, pd.NA)
    diff_pct_nike = (df['Dexter Final Price'] - df['Nike Final Price']) / nike
    df['Dexter vs Nike (BML)'] = df.apply(lambda r: _bml_label_from_prices(r.get('Dexter Final Price'), r.get('Nike Final Price')), axis=1)

    df['Dexter Shipping'] = df['Dexter Final Price'].apply(lambda x: _ship_for_price_dexter(float(x)) if pd.notna(x) else 0.0)
    ...
    df['Dexter Price + Shipping'] = df['Dexter Final Price'] + df['Dexter Shipping']

    diff_pct_ship = (df['Dexter Price + Shipping'] - df['Nike Price + Shipping']) / df['Nike Price + Shipping'].replace(0, pd.NA)
    df['BML with Shipping'] = df.apply(lambda r: _bml_label_from_prices(r.get('Dexter Price + Shipping'), r.get('Nike Price + Shipping')), axis=1)

    df['Cuotas Dexter'] = pd.to_numeric(df['Dexter_MaxCuotasSinInteres'], errors='coerce').fillna(0).astype(int)
    ...
    df['Competitor'] = 'Dexter'
    df['Last Update Competitor'] = df['Dexter_LastUpdated'].astype(str).fillna('').str.strip()

    df['Link PDP Competitor'] = df['Link PDP Competitor'].astype(str).fillna('').str.strip()

    df['Competitor Full Price'] = df['Dexter Full Price']
    df['Competitor Markdown'] = df['Dexter Markdown']
    df['Competitor Final Price'] = df['Dexter Final Price']

    df['Competitor vs Nike'] = df['Dexter vs Nike']

    df['BML Final Price'] = df['Dexter vs Nike (BML)']
    df['BML Full Price'] = df['Dexter vs Nike Full (BML)']

    df['Competitor Shipping'] = df['Dexter Shipping']
    df['Competitor Price + Shipping'] = df['Dexter Price + Shipping']

    df['Cuotas Competitor'] = pd.to_numeric(df['Cuotas Dexter'], errors='coerce').fillna(0).astype(int)
...
def write_visual_xlsx(df: pd.DataFrame, path: str):
    ...
    with pd.ExcelWriter(path, engine='xlsxwriter') as writer:
        ...
        for c in ['Dexter Full Price','Dexter Final Price','Nike Full Price','Nike Final Price',
                  'Dexter Shipping','Nike Shipping','Nike Price + Shipping','Dexter Price + Shipping']:
            set_col_format(c, money_fmt)

        for c in ['Dexter Markdown','Nike Markdown','Dexter vs Nike (Full Price)','Dexter vs Nike']:
...
def main():
    log("🚀 DEXTER vs NIKE – INDESTRUCTIBLE MODE")
        ...
        with sync_playwright() as pw:
            ...
            for style_norm in styles_chunk:
                ...
                with cache_lock:
                    ...
                    cache[style_norm]["Dexter_URL"] = url
                    cache[style_norm]["Dexter_StyleColor_Norm"] = style_norm
                    ...
                    already_has_valid = float(cache[style_norm].get("Dexter_FinalPrice", 0.0)) > VALID_PRICE_MIN
                ...
                if (not REFRESH_CACHED) and already_has_valid:
                    with cache_lock:
                        ...
                        dex_price = float(cache[style_norm].get("Dexter_FinalPrice", 0.0))
                        if nike_final:
                            ...
                            cache[style_norm]["Dexter_vs_Nike_pct"] = float(pct_over_dex)
                        else:
                            cache[style_norm]["DiffPctNike"] = None
                            cache[style_norm]["Dexter_vs_Nike_pct"] = None
                            pct_nike = 0.0
                            pct_over_dex = 0.0

                        atomic_write_json(CACHE_PATH, cache)

                    log(f"[{idx}/{len(all_styles)}] ✅ (W{worker_id}) SAFE-SKIP {style_norm} | ya cacheado (DexterFinal={dex_price:,.2f})")
                    local_processed += 1
                else:
                    ...
                    try:
                        ...
                        for _attempt in range(2):
                            try:
                                ...
                                dex_price = float((row or {}).get("Dexter_FinalPrice", 0.0))
                                break
                        ...
                        with cache_lock:
                            ...
                            cache[style_norm]["Dexter_PriceValid"] = bool(dex_price > VALID_PRICE_MIN)
                        ...
                        local_processed += 1
                        log(
                            f"   🧾 DexterFinal={dex_price:,.2f} | NikeFinal={nike_final:,.2f} | Δ%(Nike)={pct_nike:.3%} | Δ%(overDex)={pct_over_dex:.3%} | "
                            f"CuotasHabitualesMax={cache.get(style_norm, {}).get('Dexter_MaxCuotasSinInteres', 0)}"
                        )

                    except PWTimeoutError:
                        with cache_lock:
                            local_errors += 1
                            cache[style_norm]["Dexter_Error"] = "Timeout"
                            cache[style_norm]["Dexter_LastErrorAt"] = dt.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                            atomic_write_json(CACHE_PATH, cache)
                        ...
                        with cache_lock:
                            local_errors += 1
                            cache[style_norm]["Dexter_Error"] = f"{type(e).__name__}: {e}"
                            cache[style_norm]["Dexter_LastErrorAt"] = dt.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                            atomic_write_json(CACHE_PATH, cache)
    ...
    log("\n📤 Stage 3: exportando reporte desde dexter_cache.json ...")
    cache = load_cache(CACHE_PATH)
    ...
    if not df.empty and "Dexter_StyleColor_Norm" in df.columns:
        df = df.sort_values(by=["Dexter_StyleColor_Norm"], ascending=True).reset_index(drop=True)
