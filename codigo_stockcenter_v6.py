...
stockcenter_STD_SHIPPING_ARS = 6899.0
stockcenter_FREE_SHIP_FROM_ARS = 149000.0
...
AGENTS = max(3, int(os.getenv('AGENTS', '3')))  # set 1 to disable parallelism
...
CACHE_PATH = "stockcenter_cache.json"
...
OUT_RAW_CSV = f"stockcenter_vs_nike_raw_{TS}.csv"
OUT_VISUAL_XLSX = f"stockcenter_vs_nike_visual_{TS}.xlsx"
...
def scrape_pdp(page, url: str) -> dict:
    ...
    return {
        "Retailer": "stockcenter",
        "stockcenter_URL": url,
        "stockcenter_Name": name,
        "stockcenter_StyleColor_Raw": style_raw,
        "stockcenter_StyleColor_Norm": style_norm,
        "stockcenter_FullPrice": float(full_price),
        "stockcenterSaleDecimal": float(sale_dec),
        "stockcenter_FinalPrice": float(price_final),
        "stockcenter_MaxCuotasSinInteres": int(max_cuotas),
        "stockcenter_LastUpdated": dt.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
    }
...
def _ship_for_price_stockcenter(price: float) -> float:
    try:
        p = float(price)
    except Exception:
        return 0.0
    if p <= 0:
        return 0.0
    try:
        return 0.0 if p >= float(stockcenter_FREE_SHIP_FROM_ARS) else float(stockcenter_STD_SHIPPING_ARS)
    except Exception:
        return 0.0
...
def build_template_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    base_cols = [
        'stockcenter_StyleColor_Norm','SB_ProductCode', 'SB_MarketingName', 'SB_Category','SB_MarketingName','SB_Division','SB_Franchise',
        'PLP_PrimaryCategoria','stockcenter_URL',
        'stockcenter_FullPrice','stockcenterSaleDecimal','stockcenter_FinalPrice',
        'NikeFullPrice','NikeSaleDecimal','NikeFinalPrice', 'FechaCorrida',
        'stockcenter_MaxCuotasSinInteres','stockcenter_LastUpdated','SB_SSN_VTA','Aux_Año','InStatusBooks'
    ]
    for c in base_cols:
        if c not in df.columns:
            df[c] = pd.NA

    df['StyleColor'] = df['stockcenter_StyleColor_Norm'].astype(str).fillna('').str.strip()
    ...
    df['Link PDP Competitor'] = df['stockcenter_URL'].astype(str).fillna('').str.strip()

    df['stockcenter Full Price'] = pd.to_numeric(df['stockcenter_FullPrice'], errors='coerce')
    df['stockcenter Markdown'] = pd.to_numeric(df['stockcenterSaleDecimal'], errors='coerce')
    df['stockcenter Final Price'] = pd.to_numeric(df['stockcenter_FinalPrice'], errors='coerce')

    _m_full_missing = df['stockcenter Full Price'].isna() | (df['stockcenter Full Price'] <= 0)
    if _m_full_missing.any():
        df.loc[_m_full_missing, 'stockcenter Full Price'] = df.loc[_m_full_missing, 'stockcenter Final Price']
        df.loc[_m_full_missing, 'stockcenter Markdown'] = 0.0
    ...
    Stock_full = df['stockcenter Full Price'].replace(0, pd.NA)
    df['stockcenter vs Nike (Full Price)'] = (df['stockcenter Full Price'] - df['Nike Full Price']) / Stock_full

    nike_full_den = df['Nike Full Price'].replace(0, pd.NA)
    diff_pct_full_nike = (df['stockcenter Full Price'] - df['Nike Full Price']) / nike_full_den
    df['stockcenter vs Nike Full (BML)'] = df.apply(lambda r: _bml_label_from_prices(r.get('stockcenter Full Price'), r.get('Nike Full Price')), axis=1)

    Stock = df['stockcenter Final Price'].replace(0, pd.NA).replace(0, pd.NA)
    df['stockcenter vs Nike'] = (df['stockcenter Final Price'] - df['Nike Final Price']) / Stock

    nike = df['Nike Final Price'].replace(0, pd.NA)
    diff_pct_nike = (df['stockcenter Final Price'] - df['Nike Final Price']) / nike
    df['stockcenter vs Nike (BML)'] = df.apply(lambda r: _bml_label_from_prices(r.get('stockcenter Final Price'), r.get('Nike Final Price')), axis=1)

    df['stockcenter Shipping'] = df['stockcenter Final Price'].apply(lambda x: _ship_for_price_stockcenter(float(x)) if pd.notna(x) else 0.0)
    ...
    df['stockcenter Price + Shipping'] = df['stockcenter Final Price'] + df['stockcenter Shipping']

    diff_pct_ship = (df['stockcenter Price + Shipping'] - df['Nike Price + Shipping']) / df['Nike Price + Shipping'].replace(0, pd.NA)
    df['BML with Shipping'] = df.apply(lambda r: _bml_label_from_prices(r.get('stockcenter Price + Shipping'), r.get('Nike Price + Shipping')), axis=1)

    df['Cuotas stockcenter'] = pd.to_numeric(df['stockcenter_MaxCuotasSinInteres'], errors='coerce').fillna(0).astype(int)
    ...
    df['Competitor'] = 'stockcenter'
    df['Last Update Competitor'] = df['stockcenter_LastUpdated'].astype(str).fillna('').str.strip()

    df['Link PDP Competitor'] = df['Link PDP Competitor'].astype(str).fillna('').str.strip()

    df['Competitor Full Price'] = df['stockcenter Full Price']
    df['Competitor Markdown'] = df['stockcenter Markdown']
    df['Competitor Final Price'] = df['stockcenter Final Price']

    df['Competitor vs Nike'] = df['stockcenter vs Nike']

    df['BML Final Price'] = df['stockcenter vs Nike (BML)']
    df['BML Full Price'] = df['stockcenter vs Nike Full (BML)']

    df['Competitor Shipping'] = df['stockcenter Shipping']
    df['Competitor Price + Shipping'] = df['stockcenter Price + Shipping']

    df['Cuotas Competitor'] = pd.to_numeric(df['Cuotas stockcenter'], errors='coerce').fillna(0).astype(int)
...
def write_visual_xlsx(df: pd.DataFrame, path: str):
    ...
    with pd.ExcelWriter(path, engine='xlsxwriter') as writer:
        ...
        for c in ['stockcenter Full Price','stockcenter Final Price','Nike Full Price','Nike Final Price',
                  'stockcenter Shipping','Nike Shipping','Nike Price + Shipping','stockcenter Price + Shipping']:
            set_col_format(c, money_fmt)

        for c in ['stockcenter Markdown','Nike Markdown','stockcenter vs Nike (Full Price)','stockcenter vs Nike']:
...
def main():
    log("🚀 stockcenter vs NIKE – INDESTRUCTIBLE MODE")
        ...
        with sync_playwright() as pw:
            ...
            for style_norm in styles_chunk:
                ...
                with cache_lock:
                    ...
                    cache[style_norm]["stockcenter_URL"] = url
                    cache[style_norm]["stockcenter_StyleColor_Norm"] = style_norm
                    ...
                    already_has_valid = float(cache[style_norm].get("stockcenter_FinalPrice", 0.0)) > VALID_PRICE_MIN
                ...
                if (not REFRESH_CACHED) and already_has_valid:
                    with cache_lock:
                        ...
                        Stock_price = float(cache[style_norm].get("stockcenter_FinalPrice", 0.0))
                        if nike_final:
                            pct_nike = ((Stock_price - nike_final) / nike_final)
                            pct_over_Stock = ((Stock_price - nike_final) / Stock_price) if Stock_price else 0.0
                            cache[style_norm]["DiffPctNike"] = float(pct_nike)
                            cache[style_norm]["stockcenter_vs_Nike_pct"] = float(pct_over_Stock)
                        else:
                            cache[style_norm]["DiffPctNike"] = None
                            cache[style_norm]["stockcenter_vs_Nike_pct"] = None
                            pct_nike = 0.0
                            pct_over_Stock = 0.0

                        atomic_write_json(CACHE_PATH, cache)

                    log(f"[{idx}/{len(all_styles)}] ✅ (W{worker_id}) SAFE-SKIP {style_norm} | ya cacheado (stockcenterFinal={Stock_price:,.2f})")
                    local_processed += 1
                else:
                    ...
                    try:
                        ...
                        for _attempt in range(2):
                            try:
                                ...
                                Stock_price = float((row or {}).get("stockcenter_FinalPrice", 0.0))
                                break
                        ...
                        with cache_lock:
                            ...
                            cache[style_norm]["stockcenter_PriceValid"] = bool(Stock_price > VALID_PRICE_MIN)
                        ...
                        local_processed += 1
                        log(
                            f"   🧾 stockcenterFinal={Stock_price:,.2f} | NikeFinal={nike_final:,.2f} | Δ%(Nike)={pct_nike:.3%} | Δ%(overStock)={pct_over_Stock:.3%} | "
                            f"CuotasHabitualesMax={cache.get(style_norm, {}).get('stockcenter_MaxCuotasSinInteres', 0)}"
                        )

                    except PWTimeoutError:
                        with cache_lock:
                            local_errors += 1
                            cache[style_norm]["stockcenter_Error"] = "Timeout"
                            cache[style_norm]["stockcenter_LastErrorAt"] = dt.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                            atomic_write_json(CACHE_PATH, cache)
                        ...
                        with cache_lock:
                            local_errors += 1
                            cache[style_norm]["stockcenter_Error"] = f"{type(e).__name__}: {e}"
                            cache[style_norm]["stockcenter_LastErrorAt"] = dt.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                            atomic_write_json(CACHE_PATH, cache)
    ...
    log("\n📤 Stage 3: exportando reporte desde stockcenter_cache.json ...")
    cache = load_cache(CACHE_PATH)
    ...
    for style_norm, rec in cache.items():
        ...
        if not rec.get("stockcenter_URL"):
            continue
    ...
    if not df.empty and "stockcenter_StyleColor_Norm" in df.columns:
        df = df.sort_values(by=["stockcenter_StyleColor_Norm"], ascending=True).reset_index(drop=True)
